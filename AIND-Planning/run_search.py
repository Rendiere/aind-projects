...
def main(p_choices, s_choices):
    ...
    for pname, p in problems:
        ...
        for sname, s, h in searches:
            ...
            '''
            Thanks to @mark_190947389137686
            '''
            search_proc = Process(target=run_search, name = 'Run Search', args=(_p, s, _h))
            search_proc.start()
            time_cutoff_seconds = 600
            search_proc.join(time_cutoff_seconds)
            if search_proc.is_alive():
                print(f'{pname} using {sname} took longer than {time_cutoff_seconds} seconds, so was terminated')
