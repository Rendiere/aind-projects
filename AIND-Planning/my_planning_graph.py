...
def equal(l1: list, l2: list) -> bool:
    """
    Test if l1 and l2 are equal and that they are not empty
    :return:
    """
    if l1 and l1 == l2:
        return True
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Initialize empty a_levels set at index level
        self.a_levels.append(set())
        for action in self.all_actions:
            # Create a PgNode
            pg_a = PgNode_a(action)
            # Get the set of S-Nodes on this level
            pg_s_set = self.s_levels[level]
            # If this action's effects is a subset of this level's S-nodes
            if pg_a.prenodes.issubset(pg_s_set):
                # Add the S nodes as parents to this A node
                pg_a.parents.update(pg_s_set)
                # Add this action to the current level's Action set
    ...
    def add_literal_level(self, level):
        ...
        # For all actions in previous level
        for a_node in self.a_levels[level - 1]:
            # Add effects of actions at previous level as S-nodes to this level
            self.s_levels[level].update(a_node.effnodes)
            # Connect effects as children to A-node
            a_node.children.update(self.s_levels[level])

            # For each literal effect of this action
            for effect_S in a_node.effnodes:
                # Add the effect S node to the current S level
                self.s_levels[level].add(effect_S)
                # Connect this S node as a child of the action that created it
                a_node.children.add(effect_S)
                # Connect the action as a parent of this S node
                effect_S.parents.add(a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if ((node_a1.action.effect_add == node_a2.action.effect_rem)
            or (node_a2.action.effect_add == node_a1.action.effect_rem)):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if (equal(action_1.effect_add, action_2.precond_neg) or equal(action_1.effect_rem,action_2.precond_pos) or
            equal(action_2.effect_add , action_1.precond_neg) or equal(action_2.effect_rem, action_1.precond_pos)):
            return True
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        if node_s1.is_pos and not node_s2.is_pos and (node_s1.symbol == node_s2.symbol):
            return True

        if node_s2.is_pos and not node_s1.is_pos and (node_s1.symbol == node_s2.symbol):
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for g in self.problem.goal:
            ...
            for i, s_level in enumerate(self.s_levels):
                ...
                if any([True for s in s_level if s.is_pos and s.symbol == g]):
                    level_sum += i
                    break
